document.addEventListener('DOMContentLoaded', function() {
    ...
    const inputTextarea = document.getElementById('sha512-input');
    const convertedTextarea = document.getElementById('sha512-converted');

    btnConvert.addEventListener('click', async function() {
        const inputText = inputTextarea.value;
        const convertedText = await sha512Encode(inputText);
    ...
    async function sha512Encode(input) {
